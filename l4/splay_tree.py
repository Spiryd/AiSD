...
class SplayTree:
	...
	def __print_tree_helper(self, node, indent, is_last):
		if node is not None:
			marker = "└── " if is_last else "├── "
			print(indent + marker + str(node.data))

			child_indent = indent + ("    " if is_last else "│   ")
			self.__print_tree_helper(node.left, child_indent, False)
			self.__print_tree_helper(node.right, child_indent, True)
		else:
